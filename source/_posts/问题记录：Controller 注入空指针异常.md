...
问题出现了，原有的 "/test/pri" 接口在调用 "testService#say" 方法时报了“空指针异常”，而新增的 "/test/pub" 却能正常使用。这乍一看，着实有点诡异，于是便打开 IDEA  debug 一波。
...
发现代理类中只代理只实现了 testPublic 方法，到这里这个问题的原因便清晰了。

1. 首先是出现问题的这个服务中有一个全局切面类，它针对所有的 Controller 的方法增加了日志打印的处理逻辑；
2. 当 Controller 中只有私有方法时，CGLIB 不会生成该 Controller 的代理类，此时不会有任何问题；
3. 当  Controller 中存在公有方法时，CGLIB 将会生成该 Controller 的代理类，并且完成 bean 注入流程的实际是被代理的目标对象；
4. 当请求过来时，对 Controller 方法的调用实际是对 CGLIB 代理对象的调用，最终转化为对目标对象的调用，但同时另一方面，由于 CGLIB 不会代理私有方法，所以此时也不会转化为目标对象的调用，问题便出现了。
